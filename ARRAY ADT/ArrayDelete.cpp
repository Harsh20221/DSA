...
void DeleteArray(struct Array *arr , int index ){
    if ( index>=0 && index<=arr->length){
        for ( int i = index ; i<arr->length-1 ; i++) // Fix the loop condition // Remember to increase  the loop variable , make sure that initially i will be equal to array's index  and then the loop will continue i is smaller than array's length-1    , we subract 1 from the length of the array because we are shifting the  elements towards right 
        arr->A[i] = arr->A[i+1]; // Shifting the elements to the lrft side from  0 to length of the array   //! In this loop we are moving i from left to right because we are increasing i but we are shifting elements from right to left because we are decreasing the index of the array elements
